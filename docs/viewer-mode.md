...
- Part 설명, 믹서, 설정은 사이드바 또는 플로팅 패널에 숨긴다.
- 하단에는 고정 재생바를 둔다.
...
│ auto-scroll                                  │                 │
├──────────────────────────────────────────────┴─────────────────┤
│ Playbar: play, seek, time, loop, mixer, parts                   │
└────────────────────────────────────────────────────────────────┘
```
...
const activeCues = cues.filter(cue =>
  cue.startMs <= currentMs && currentMs < cue.endMs
)
```
...
- active variant volume/mute/solo
- 전체 mute/solo 상태
