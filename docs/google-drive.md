...
3. `Drive 열기`를 누른다.
4. Google 로그인을 완료하면 Drive metadata와 파일 내용을 가져온다.
5. Drive capability에 따라 편집 가능 파일은 Editor로, 다운로드만 가능한 파일은 Practice Viewer로 열린다.
...
1. Project File 섹션에서 `Drive 선택`을 누른다.
2. Google 로그인을 완료한다.
