...
Eazy Chorus는 백엔드 없이 브라우저에서 Google Drive API를 직접 호출한다. OAuth access token은 브라우저 런타임에서만 사용하며, 만료 전까지 브라우저 저장소에 캐시한다. 서버 저장소나 refresh token은 사용하지 않는다.
...
3. `Drive 열기`를 누른다.
4. 처음 사용하거나 cached access token이 만료된 경우 Google 로그인을 완료한다.
...
- access token cache는 브라우저 저장소에 보관되며, 만료 1분 전부터는 재사용하지 않는다.
...
- 공유 링크로 보기 전용 파일 열기
- 편집 권한 파일을 `Drive에 저장`
- 같은 브라우저에서 access token 만료 전 재열기 시 로그인 요청 생략
